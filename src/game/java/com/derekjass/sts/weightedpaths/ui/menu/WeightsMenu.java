...
public class WeightsMenu implements RenderSubscriber, PostUpdateSubscriber {
    ...
    /** Left-aligned labels; controls sit to the right so Chinese text is not clipped. */
    private static final float labelX = Legend.X + 8.0f;
    private static final float controlX = Legend.X + 118.0f;
    ...
    private WeightsMenu() {
        ...
        float rowY = menuY;
        createWidgetRow(rowY, ModUiStrings.LABEL_STORE, "$");
        rowY += ySpacing;
        createWidgetRow(rowY, ModUiStrings.LABEL_REST, "R");
        rowY += ySpacing;
        createWidgetRow(rowY, ModUiStrings.LABEL_UNKNOWN, "?");
        rowY += ySpacing;
        createWidgetRow(rowY, ModUiStrings.LABEL_MONSTER, "M");
        rowY += ySpacing;
        createWidgetRow(rowY, ModUiStrings.LABEL_ELITE, "E");
    }

    private void createWidgetRow(float y, String label, String nodeType) {
        float weightX = controlX + LEFT_ARROW.getWidth() + ((arrowXSpacing - LEFT_ARROW.getWidth()) / 2);
        renderables.add(new LabelText(labelX, y, label));
        renderables.add(new WeightSelector(LEFT_ARROW, controlX, y, nodeType, false));
        renderables.add(new WeightText(weightX, y, nodeType));
        renderables.add(new WeightSelector(RIGHT_ARROW, controlX + arrowXSpacing, y, nodeType, true));
    }
