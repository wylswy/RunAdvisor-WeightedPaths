...
public class LowHpRestRuleTest {
    ...
    public void hpBelowThirtyPercentForcesRest() {
        ...
        assertTrue("15/70 应触发强制火堆", WeightedPaths.shouldForceRestRoute(15, 70));
    }

    @Test
    public void hpAtThirtyPercentDoesNotForce() {
        // 21/70 = 0.30，等于阈值 → 不应强制（严格小于）
        assertFalse("21/70 等于阈值不应触发", WeightedPaths.shouldForceRestRoute(21, 70));
    }

    @Test
    public void healthyHpDoesNotForce() {
        assertFalse("55/70 健康不应触发", WeightedPaths.shouldForceRestRoute(55, 70));
    }

    @Test
    public void zeroHpEdgeCase() {
        // 死亡边缘（currentHp=0）不应触发强制（避免在已死亡状态误判）
        assertFalse("currentHp=0 不应触发", WeightedPaths.shouldForceRestRoute(0, 70));
    }

    @Test
    public void zeroMaxHpEdgeCase() {
        // maxHp=0 非法状态，不应触发
        assertFalse("maxHp=0 不应触发", WeightedPaths.shouldForceRestRoute(10, 0));
    }
}
