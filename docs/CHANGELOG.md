...
| 提交 | 变更 | 原因 |
|------|------|------|
| （8-11 实测收尾 · 2026-08-11） | 实测验证：契约完成/违背/记仇/奖励情报闭环、聊天按钮可用、打字恢复；修复 agent_log need 字段非法 JSON、AgentCoreTest 污染真实日志（日志目录可注入+测试隔离）、聊天框长消息换行、探针 per-run 口径；下架「别再抓攻击牌」契约（只留「五成血到精英」）；第二版 eval：7 局/遵循率 57.9%/AI 平均延迟 2.2s/兜底率 50% | 把「验证闭环」扣上：主链路实测通过 + 数据 bug 清零 + 评估有数 |`n| （交付链路收口 · 2026-08-11） | CI 加打包发布（game-test 全量测试后 mvn package + 上传 WeightedPaths.jar artifact；打 * tag 自动发 GitHub Release）；新增 docs/KNOWN-ISSUES.md（开放问题唯一真源）、docs/RECOVERY.md（换机恢复步骤）；run_mvn.bat 改 %~dp0 相对路径 + JAVA_HOME/M2_HOME 环境变量可覆盖；DEV-ROADMAP 挂 KNOWN-ISSUES、修过时 bug 行 | 把「通过测试」变成「产出可部署 jar」；让「项目缺什么 / 怎么恢复」一查即得 |
| （原子写 + 实测清单 · 2026-08-11） | run_state.json 改临时文件+rename 原子写（崩溃/断电不再半截损坏）；agent_log.json 追加前补换行隔离半截残留行；补无残留测试；新增 docs/PLAYTEST-CHECKLIST.md 进游戏实测清单（契约 Act3 结算/SL 契约保留/AiExecutor/数据审计等主链路逐项验证） | 多文件持久化的一致性兜底 + 给「接入之后」的验证一个可执行的入口 |
| （CI 门禁拆分 · 2026-08-11） | CI 拆双 job：pure-test（mvn -Ppure，纯逻辑 209 测试无需游戏 jar、永远跑，门禁永不空转）+ game-test（需游戏 jar，未配置 secret 时跳过）；纯逻辑/游戏层源码分目录（src/main/java 纯逻辑 + src/game/java 游戏层 + src/game-test/java 游戏测试），pom 加 game（activeByDefault）/pure 双 profile；PactManager 精英情报改注入式；PathSymbolCounts 拆纯逻辑 + NodePathSymbolCounts（游戏层 fromNodes）；低血保命规则抽 RouteSafetyRules 纯类；log4j-api 以 provided 声明 | 让 CI 门禁永不空转：无游戏 jar 也跑大部分测试；层边界显式化，接线层判定下沉可测 |
